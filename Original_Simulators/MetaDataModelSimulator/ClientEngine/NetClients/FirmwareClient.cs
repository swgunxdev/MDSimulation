...
{
	public class FirmwareClient// : TCPAsyncClient
	{
		private TcpListener _Listener = null;
		private TcpClient _Client = null;
		private int _Port = 0;
		private byte[] _Buffer = new byte[0x200000];
		...
		public void StartListening (int firmwarePort)
		{
			_Port = firmwarePort;
			...
			Console.WriteLine(string.Format("TCP Firmware handler accepting on port {0}", _Port));
		}
		...
		public void StopListening()
		{
			...
			Console.WriteLine("TCP Firmware handler stopped");
		}
		...
		private void OnAcceptConnection(IAsyncResult ar)
		{
			...
			if (listener != null)
			{
				try
				{
					...
					Console.WriteLine("TCP Firmware accepted connection. listening for more");
		...
		private void HandleData(int bytesReceived)
		{
			...
			Console.WriteLine(string.Format("TCP Firmware handler received {0} bytes.", bytesReceived));
