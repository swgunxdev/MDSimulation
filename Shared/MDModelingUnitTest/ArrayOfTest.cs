...
namespace MDModelingUnitTest
{
    ...
    public class ArrayOfTest
    {
            ...
            where T : struct
        {
            ...
            ArrayOfProperty<T> target = new ArrayOfProperty<T>(id, typeId, name, value, maxSize);;
        ...
        {
            Assert.That(() => ConstructorTestHelper<int>(new int[] { 12, 34, 56 }, 2), Throws.TypeOf<ArgumentOutOfRangeException>());
            Assert.That(() => ConstructorTestHelper<double>(new double[] { 12E+3, 34E-23, 56E-2, }, 1), Throws.TypeOf<ArgumentOutOfRangeException>());
            ConstructorTestHelper<float>(new float[] { 12.0f, 34.0f, 56.0f }, 3);
            ConstructorTestHelper<byte>(new byte[] { 12, 34, 56, 45, 12, 255 }, 0);
        }
